...
app = FastAPI(title="Rule 115 — SHIFT Requires MODE", version="1.0")
...
class Unit(BaseModel):
    ...
    code: Optional[str] = ""
    # results key will be added dynamically: rule115_findings
...
def snippet_at(text: str, start: int, end: int) -> str:
    """Return a short, escaped snippet for context."""
    s = max(0, start - 60)
    ...
    for m in STMT_RE.finditer(src):
        ...
        if not has_mode:
            findings.append({
                ...
                "issue_type": "ShiftWithoutMode",
                "severity": "warning",
                "line": line_of_offset(src, start),
                "message": "SHIFT without MODE. Specify IN CHARACTER MODE (text) or IN BYTE MODE (binary).",
                "suggestion": (
                    "SHIFT <var> LEFT|RIGHT BY <n> PLACES IN CHARACTER MODE.\n"
                    "* or *\n"
                    "SHIFT <xvar> LEFT|RIGHT BY <n> PLACES IN BYTE MODE.Specify IN CHARACTER MODE (text) or IN BYTE MODE (binary)."
                ),
                "snippet": snippet_at(src, start, end),
            })

    obj = unit.model_dump()
    obj["rule115_findings"] = findings
    return obj
...
async def scan_rule(units: List[Unit]):
    ...
    for u in units:
        res = scan_unit(u)
        if res.get("rule115_findings"):
            results.append(res)
    return results

@app.get("/health")
async def health():
    return {"ok": True, "rule": 115}
